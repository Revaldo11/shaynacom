        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.abouts.update', $about) }}" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        <x-input-label :value="__('Keypoints')" />
                        <div class="flex flex-col gap-y-5">
                            @foreach ($about->keypoints as $keypoint)
                                <input type="text" name="keypoints[{{ $keypoint->id }}]"
                                    value="{{ old('keypoints.' . $keypoint->id, $keypoint->keypoint) }}"
                                    class="py-3 rounded-lg border border-slate-300">
                            @endforeach
                        </div>
