...
@section('content')
    <div id="header" class="bg-[#F6F7FA] relative overflow-hidden">
        <div class="container max-w-[1130px] mx-auto relative pt-10 z-10">
            ...
            <div id="Hero" class="flex flex-col gap-[30px] mt-20 pb-20">
                @forelse ($hero_section as $item)
                    <input type="hidden" name="path_video" id="path_video" value="{{ $item->path_video }}">
                    <div class="flex items-center bg-white p-[8px_16px] gap-[10px] rounded-full w-fit">
                        ...
                        <p class="font-semibold text-sm">{{ $item->achievement }}</p>
                    </div>
                    <div class="flex flex-col gap-[10px]">
                        <h1 class="font-extrabold text-[50px] leading-[65px] max-w-[536px]">{{ $item->heading }}</h1>
                        <p class="text-cp-light-grey leading-[30px] max-w-[437px]">{{ $item->subHeading }}</p>
                    </div>
        ...
        <div class="absolute w-[43%] h-full top-0 right-0 overflow-hidden z-0">
            <img src="{{ Storage::url($item->banner) }}" class="object-cover w-full h-full" alt="banner">
        </div>
    </div>
            ...
            @forelse ($principles as $item)
                <div
                    ...
                    <div class="thumbnail h-[200px] flex shrink-0 overflow-hidden">
                        <img src="{{ Storage::url($item->thumbnail) }}" class="object-cover object-center w-full h-full"
                            alt="thumbnails">
                    </div>
                    <div class="flex flex-col p-[0_30px_30px_30px] gap-5">
                        <div class="w-[55px] h-[55px] flex shrink-0 overflow-hidden">
                            <img src="{{ Storage::url($item->icon) }}" class="w-full h-full object-contain"
                                alt="icon">
                        </div>
                        <div class="flex flex-col gap-1">
                            <p class="title font-bold text-xl leading-[30px]">{{ $item->name }}</p>
                            <p class="leading-[30px] text-cp-light-grey">{{ $item->subtitle }}</p>
                        </div>
                        <a href="" class="font-semibold text-cp-dark-blue">Learn More</a>
                    </div>
                </div>
            @empty
                <p>Belum ada data principle</p>
            @endforelse
        </div>
    </div>
            ...
            <div class="flex flex-wrap items-center justify-between p-[10px]">
                @forelse ($statistics as $item)
                    <div class="card w-[200px] flex flex-col items-center gap-[10px] text-center">
                        <div class="w-[55px] h-[55px] flex shrink-0 overflow-hidden">
                            <img src="{{ Storage::url($item->icon) }}" class="object-contain w-full h-full"
                                alt="icon">
                        </div>
                        <p class="text-cp-pale-orange font-bold text-4xl leading-[54px]">{{ $item->goal }}</p>
                        <p class="text-cp-light-grey">{{ $item->name }}</p>
                    </div>
                @empty
                    <p>Belum ada data</p>
                @endforelse
        ...
        @forelse ($products as $item)
            <div class="product flex flex-wrap justify-center items-center gap-[60px] even:flex-row-reverse">
                <div class="w-[470px] h-[550px] flex shrink-0 overflow-hidden">
                    <img src="{{ Storage::url($item->thumbnail) }}" class="w-full h-full object-contain"
                        alt="thumbnail">
                </div>
                <div class="flex flex-col gap-[30px] py-[50px] h-fit max-w-[500px]">
                    <p
                        ...
                        {{ $item->tagline }}</p>
                    <div class="flex flex-col gap-[10px]">
                        <h2 class="font-bold text-4xl leading-[45px]">{{ $item->name }}
                        </h2>
                        <p class="leading-[30px] text-cp-light-grey">{{ $item->about }}</p>
                    </div>
                    <a href=""
                ...
                @forelse ($teams as $item)
                    <div
                        ...
                        <div class="flex flex-col gap-1 text-center">
                            <p class="font-bold text-xl leading-[30px]">{{ $item->name }}</p>
                            <p class="text-cp-light-grey">{{ $item->occupation }}</p>
                        </div>
                        <div class="flex items-center justify-center gap-[10px]">
                            <div class="w-6 h-6 flex shrink-0">
                                <img src="assets/icons/global.svg" alt="icon">
                            </div>
                            <p class="text-cp-dark-blue font-semibold">{{ $item->location }}</p>
                        </div>
                    </div>
                @empty
                    <p>Tidak ada data terbaru</p>
            ...
            @forelse ($testimonials as $item)
                <div
                    ...
                    <div class="testimonial-container flex flex-col gap-[112px] w-[565px]">
                        <div class="flex flex-col gap-[30px]">
                            <div class="h-9 overflow-hidden">
                                <img src="{{ Storage::url($item->client->logo) }}" class="object-contain"
                                    alt="icon">
                            </div>
                            <div class="relative pt-[27px] pl-[30px]">
                                ...
                                <p class="font-semibold text-2xl leading-[46px] relative z-10">{{ $item->message }}</p>
                            </div>
                            <div class="flex items-center justify-between pl-[30px]">
                                <div class="flex items-center gap-6">
                                    <div class="w-[60px] h-[60px] flex shrink-0 rounded-full overflow-hidden">
                                        <img src="{{ Storage::url($item->client->avatar) }}"
                                            class="w-full h-full object-cover" alt="photo">
                                    </div>
                                    <div class="flex flex-col justify-center gap-1">
                                        <p class="font-bold">{{ $item->client->name }}</p>
                                        <p class="text-sm text-cp-light-grey">{{ $item->client->occupation }}</p>
                                    </div>
                                </div>
                    ...
                    <div class="testimonial-thumbnail w-[470px] h-[550px] rounded-[20px] overflow-hidden bg-[#D9D9D9]">
                        <img src="{{ Storage::url($item->thumbnail) }}" class="w-full h-full object-cover object-center"
                            alt="thumbnail">
                    </div>
                </div>
            @empty
